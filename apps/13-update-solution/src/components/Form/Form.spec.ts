...
const renderForm = (person?: Person) =>
  render(Form, {
    props: person ? { person } : {},
    global: {
      stubs: {
...
describe('Form.vue', () => {
  it('renders the default avatar and empty form fields in create mode', () => {
    renderForm();
  ...
  it('prefills fields from person prop when switching to edit mode', async () => {
  ...
  it('updates the form fields when the person prop changes multiple times', async () => {
    const updatedPerson = {
      ...PERSON,
      firstname: 'Grace',
      lastname: 'Hopper',
    ...
    await rerender({ person: PERSON });
    await rerender({ person: updatedPerson });

    expect(screen.getByPlaceholderText('Enter your firstname')).toHaveValue(updatedPerson.firstname);
    expect(screen.getByPlaceholderText('Enter your lastname')).toHaveValue(updatedPerson.lastname);
    expect(screen.getByPlaceholderText('Enter your email')).toHaveValue(updatedPerson.email);
    expect(screen.getByPlaceholderText('Enter your phone')).toHaveValue(updatedPerson.phone);
    expect(screen.getByAltText('person-form-avatar')).toHaveAttribute('src', updatedPerson.photo);
  });
});
