...
describe('Details.vue', () => {
  ...
  it('reloads person details when route id changes', async () => {
    const secondPerson = { ...PERSON, id: 'person-2', firstname: 'Castaneda' };
    getPersonDetails = vi.fn(async (id: string) => (id === secondPerson.id ? secondPerson : PERSON));
    const { rerender } = renderDetails();

    await rerender({ id: secondPerson.id });

    await waitFor(() => {
      expect(getPersonDetails).toHaveBeenCalledWith(secondPerson.id);
      expect(screen.getByTestId('details-form-person')).toHaveTextContent(secondPerson.firstname);
    });
  });
