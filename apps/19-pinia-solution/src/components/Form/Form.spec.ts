  ...
  render(Form, {
    ...
    global: {
      stubs: {
        ...
        'a-form-item': {
          props: ['label', 'help', 'validateStatus'],
          template: '<label><span>{{ label }}</span><slot /><span v-if="help" :data-status="validateStatus">{{ help }}</span></label>',
        },
        'a-input': {
          props: ['value', 'placeholder', 'name'],
          emits: ['update:value', 'blur'],
          template:
            '<input :name="name" :value="value" :placeholder="placeholder" @input="$emit(\'update:value\', $event.target.value)" @blur="$emit(\'blur\')" />',
        },
        'a-button': {
          props: ['disabled'],
          emits: ['click'],
...
describe('Form.vue', () => {
  it('renders the default avatar and empty form fields', async () => {
  ...
  it('emits close with the current form payload when save is clicked with a valid form', async () => {
    const { emitted } = renderForm();
    ...
    await fireEvent.update(screen.getByPlaceholderText('Enter your email'), 'ada.l@sfeir.com');
    ...
    expect((emitted().close?.[0] as Array<PersonForm>)?.[0]).toEqual({
      ...
      firstname: 'Ada',
      lastname: 'Lovelace',
      email: 'ada.l@sfeir.com',
      phone: '0102030405',
    });
  });

  it('keeps save disabled and shows an error when email is not a sfeir address', async () => {
    const { emitted } = renderForm({ ...PERSON, firstname: '', lastname: '', email: '', phone: '' });

    await fireEvent.update(screen.getByPlaceholderText('Enter your firstname'), 'Ada');
    await fireEvent.update(screen.getByPlaceholderText('Enter your lastname'), 'Lovelace');
    await fireEvent.update(screen.getByPlaceholderText('Enter your email'), 'ada@example.test');
    await fireEvent.blur(screen.getByPlaceholderText('Enter your email'));
    await fireEvent.update(screen.getByPlaceholderText('Enter your phone'), '0102030405');

    await waitFor(() => {
      expect(screen.getByText('The email must be a valid SFEIR email')).toHaveAttribute('data-status', 'error');
      expect(screen.getByRole('button', { name: 'Save' })).toBeDisabled();
    });
