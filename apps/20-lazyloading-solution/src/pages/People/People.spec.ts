import { fireEvent, render, screen, waitFor } from '@testing-library/vue';
import { vi } from 'vitest';
import { ref } from 'vue';
import type { Person, PersonForm } from '../../model/person';
import People from './People.vue';

const person: Person = {
  id: 'person-1',
  photo: 'https://example.test/photo-1.png',
  firstname: 'Leanne',
  lastname: 'Woodard',
  entity: 'BIOSPAN',
  entryDate: '27/10/2015',
  birthDate: '02/01/1974',
  gender: '',
  email: 'leanne.woodard@biospan.test',
  skills: ['vue', 'testing'],
  geo: {
    lat: 48.8541,
    lng: 2.2486,
  },
  phone: '0784112248',
  address: {
    street: 'Narrows Avenue',
    postalCode: 70534,
    city: 'Boling',
  },
  links: {
    twitter: 'https://twitter.com/leanne',
    slack: 'https://slack.com/leanne',
    github: 'https://github.com/leanne',
    linkedin: 'https://www.linkedin.com/in/leanne',
  },
  isManager: false,
  manager: 'Erika',
  managerId: 'person-manager',
};
const nextPerson: Person = {
  id: 'person-2',
  photo: 'https://example.test/photo-2.png',
  firstname: 'Castaneda',
  lastname: 'Salinas',
  entity: 'METROZ',
  entryDate: '04/10/2015',
  birthDate: '22/01/1963',
  gender: '',
  email: 'castaneda.salinas@metroz.test',
  skills: ['composition', 'props'],
  geo: {
    lat: 48.8598,
    lng: 2.2836,
  },
  phone: '0145652522',
  address: {
    street: 'Metrotech Courtr',
    postalCode: 53292,
    city: 'Franklin',
  },
  links: {
    twitter: 'https://twitter.com/castaneda',
    slack: 'https://slack.com/castaneda',
    github: 'https://github.com/castaneda',
    linkedin: 'https://www.linkedin.com/in/castaneda',
  },
  isManager: false,
  manager: 'Erika',
  managerId: 'person-manager',
};

let filteredPeopleRef: ReturnType<typeof ref<Person[]>>;
let searchRef: ReturnType<typeof ref<string>>;
let getPeople: ReturnType<typeof vi.fn>;
let deletePerson: ReturnType<typeof vi.fn>;
let createPerson: ReturnType<typeof vi.fn>;

vi.mock('../../composable/people.store', () => ({
  usePeopleStore: () => ({
    filteredPeople: filteredPeopleRef,
    search: searchRef,
    getPeople,
    deletePerson,
    createPerson,
  }),
}));

describe('People.vue', () => {
  beforeEach(() => {
    filteredPeopleRef = ref<Person[]>([]);
    searchRef = ref('');
    getPeople = vi.fn(async () => {
      filteredPeopleRef.value = [person, nextPerson];
    });
    deletePerson = vi.fn();
    createPerson = vi.fn();
  });

  const renderPeople = () =>
    render(People, {
      global: {
        stubs: {
          Card: {
            props: ['person'],
            emits: ['onDeletePerson'],
            template: `<div>
              <div data-testid="person-card">{{ person?.firstname }} {{ person?.lastname }}</div>
              <button type="button" data-testid="delete-person" @click="$emit('onDeletePerson', person)">delete</button>
            </div>`,
          },
          Form: {
            template: `<div data-testid="person-form">
              <button type="button" data-testid="cancel-add-person" @click="$emit('close')">cancel</button>
              <button
                type="button"
                data-testid="save-add-person"
                @click="$emit('close', {
                  id: '',
                  photo: 'https://randomuser.me/api/portraits/lego/6.jpg',
                  firstname: 'Ada',
                  lastname: 'Lovelace',
                  email: 'ada@example.test',
                  phone: '0102030405'
                })"
              >
                save
              </button>
            </div>`,
          },
          Search: {
            props: ['modelValue'],
            emits: ['update:modelValue'],
            template: `<div data-testid="people-search">
              <span data-testid="search-value">{{ modelValue }}</span>
              <button type="button" data-testid="update-search" @click="$emit('update:modelValue', 'cast')">update-search</button>
            </div>`,
          },
          'a-float-button': {
            template: '<button type="button" data-testid="open-modal" @click="$emit(\'click\')"><slot name="icon" /></button>',
          },
          'a-modal': {
            props: ['open'],
            template: '<div data-testid="add-person-modal" :data-open="String(open)"><slot /></div>',
          },
          PlusOutlined: {
            template: '<i data-testid="plus-icon" />',
          },
        },
      },
    });

  it('loads people on mount and renders a card per person', async () => {
    renderPeople();

    expect(getPeople).toHaveBeenCalledTimes(1);

    await waitFor(() => {
      expect(screen.getByText(`${person.firstname} ${person.lastname}`)).toBeInTheDocument();
      expect(screen.getByText(`${nextPerson.firstname} ${nextPerson.lastname}`)).toBeInTheDocument();
    });

    expect(screen.getAllByTestId('person-card')).toHaveLength(2);
  });

  it('calls deletePerson when a card emits the delete event', async () => {
    renderPeople();

    await waitFor(() => {
      expect(screen.getAllByTestId('person-card')).toHaveLength(2);
    });

    const deleteButtons = screen.getAllByTestId('delete-person');
    await fireEvent.click(deleteButtons[1]);

    expect(deletePerson).toHaveBeenCalledTimes(1);
    expect(deletePerson).toHaveBeenCalledWith(nextPerson.id);
  });

  it('opens the add person modal when clicking the floating button', async () => {
    renderPeople();

    const modal = screen.getByTestId('add-person-modal');
    expect(modal).toHaveAttribute('data-open', 'false');
    expect(screen.getByTestId('person-form')).toBeInTheDocument();

    await fireEvent.click(screen.getByTestId('open-modal'));

    await waitFor(() => {
      expect(modal).toHaveAttribute('data-open', 'true');
    });
  });

  it('closes the modal without creating a person when form emits close without payload', async () => {
    renderPeople();

    const modal = screen.getByTestId('add-person-modal');
    await fireEvent.click(screen.getByTestId('open-modal'));
    await waitFor(() => {
      expect(modal).toHaveAttribute('data-open', 'true');
    });

    await fireEvent.click(screen.getByTestId('cancel-add-person'));

    await waitFor(() => {
      expect(modal).toHaveAttribute('data-open', 'false');
    });
    expect(createPerson).not.toHaveBeenCalled();
  });

  it('creates a person and closes the modal when form emits close with payload', async () => {
    createPerson = vi.fn(async (_person: PersonForm) => undefined);
    renderPeople();

    const modal = screen.getByTestId('add-person-modal');
    await fireEvent.click(screen.getByTestId('open-modal'));
    await waitFor(() => {
      expect(modal).toHaveAttribute('data-open', 'true');
    });

    await fireEvent.click(screen.getByTestId('save-add-person'));

    expect(createPerson).toHaveBeenCalledTimes(1);
    expect(createPerson).toHaveBeenCalledWith({
      id: '',
      photo: 'https://randomuser.me/api/portraits/lego/6.jpg',
      firstname: 'Ada',
      lastname: 'Lovelace',
      email: 'ada@example.test',
      phone: '0102030405',
    });
    await waitFor(() => {
      expect(modal).toHaveAttribute('data-open', 'false');
    });
  });

  it('updates store search when the search component emits a model update', async () => {
    renderPeople();

    expect(screen.getByTestId('search-value')).toHaveTextContent('');
    await fireEvent.click(screen.getByTestId('update-search'));
    expect(searchRef.value).toBe('cast');
  });
});
