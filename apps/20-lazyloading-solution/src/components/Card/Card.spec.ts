...
const renderCard = (personOverride: Person = person, overrideProps: Partial<{ onOnDeletePerson: (person: Person) => void }> = {}) =>
  render(Card, {
    props: {
...
describe('Card.vue', () => {
  ...
  it('renders the manager when available', () => {
    renderCard();

    const manager = screen.getByText(person.manager);
    expect(manager).toBeInTheDocument();
    expect(manager).toHaveStyle({ fontStyle: 'italic' });
  });

  it('renders N/A when manager is missing', () => {
    renderCard({ ...person, manager: '' });

    expect(screen.getByText('N/A')).toBeInTheDocument();
  });

  it('does not apply italic style when the person is a manager', () => {
    renderCard({ ...person, isManager: true });

    expect(screen.getByText(person.manager)).not.toHaveStyle({ fontStyle: 'italic' });
  });

  it('emits onDeletePerson when the delete action is clicked', async () => {
    const onDeletePerson = vi.fn();
    renderCard(person, { onOnDeletePerson: onDeletePerson });
