...
describe('Home.vue', () => {
  beforeEach(() => {
    ...
    getRandomPerson = vi.fn(async () => PEOPLE[1]);
  });
  ...
  it('loads people on mount and renders the first person', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(PEOPLE[0].entity)).toBeInTheDocument();
    });
  });
  ...
  it('renders the avatar and action buttons', async () => {
    ...
    await waitFor(() => {
      expect(avatar).toHaveAttribute('src', PEOPLE[0].photo);
  ...
  it('picks a new person when clicking the delete action', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(PEOPLE[1].entity)).toBeInTheDocument();
    });
  });
});
