        ...
        <form>
          ...
          <md-field :class="{ 'md-invalid': $v.person.email.$error }">
            ...
            >
              ...
              <span
                v-show="$v.person.email.required && !$v.person.email.sfeirMail"
              >
                Le chanps doit correspondre à une addresse email valide (ex:
          ...
          <md-field :class="{ 'md-invalid': $v.person.phone.$error }">
            ...
            <div
              class="md-error"
              v-if="$v.person.phone.$error && $v.person.phone.$dirty"
            >
              <span v-show="!$v.person.phone.required">Champs obligatoire</span>
              <span
                v-show="
                  $v.person.phone.required && !$v.person.phone.phoneFormat
                "
              >
                Le champs doit être composée de 10 chiffres
              </span>
            </div>
          </md-field>
        </form>
...
import { CustomPersonValidators } from './validators/validators';
export default {
  props: {
    person: {
      type: Object,
  ...
  validations: {
    person: {
      ...
      email: {
        required,
        sfeirMail: CustomPersonValidators.sfeirEmailValidator,
      },
      phone: {
        required,
        phoneFormat: CustomPersonValidators.phoneFormat(/^\d{10}$/),
      },
    },
  },
  computed: {
